...
| 8 | MT5 Python package is Windows-only | Linux/macOS users blocked from live data | CSV/Parquet path is first-class; engine never imports MT5 at module level | OPEN (design) |
| 9 | MT5 terminal caps history returned per request | short backtests | chunked `copy_rates_range` + "max bars in chart" instructions in Phase 1 docs | OPEN |
